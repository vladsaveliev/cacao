...
if(cacao_report[['mode']] %in% c('hereditary', 'germline')){
...
rows = cacao_report[['loci']][['hereditary']][['high']]
if (nrow(rows) == 0) {
  cat("No regions to report within the high coverage range")  
} else {
  loci_high_coverage <- crosstalk::SharedData$new(rows)
  crosstalk::bscols(
    list(
...
rows = cacao_report[['loci']][['hereditary']][['callable']]
if (nrow(rows) == 0) {
  cat("No regions to report within the callable coverage range")  
} else {
  loci_callable <- crosstalk::SharedData$new(rows)
  crosstalk::bscols(
    list(
...
rows = cacao_report[['loci']][['hereditary']][['low']]
if (nrow(rows) == 0) {
  cat("No regions to report within the low coverage range")  
} else {
  loci_low_coverage <- crosstalk::SharedData$new(rows)
  crosstalk::bscols(
    list(
...
rows = cacao_report[['loci']][['hereditary']][['no_coverage']]
if (nrow(rows) == 0) {
  cat("No regions to report within the no-coverage range")  
} else {
  loci_no_coverage <- crosstalk::SharedData$new(rows)
  crosstalk::bscols(
    list(
